...
drawing_info = all_drawings %>%
   ...
   select(ID_Database, better_filename, fileNameShort, fullPath, category, condition)
...
# copy included drawings and rename
```{r}
file.copy(fs::path(drawing_path, drawings_to_include$fullPath), fs::path(filtered_dataset_path, drawings_to_include$better_filename))
```

```{r}

drawings_by_category <- drawings_to_include %>%
