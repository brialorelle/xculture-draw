...
```{R}
tracing_score_by <- glmer(cbind(num_correct, num_incorrect) ~ condition*scale(age_numeric)*site +
                      scale(avg_tracing_score) +
                      scale(effort_compiled) + 
                      (condition | unique_ids) +
                      (condition*scale(age_numeric) | category),
...
```{r}
lmer(data = tracing_scores_indiv, avg_tracing_score ~ scale(age_numeric)*site + (1|unique_ids))
```
