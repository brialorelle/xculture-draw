...
```{r}
## Extra models after peer-review
### composite effort score

d <- d %>%
  ungroup %>%
  mutate(avg_tracing_score_scaled = scale(avg_tracing_score),
         mean_intensity_scaled = scale(mean_intensity),
         draw_duration_scaled = scale(draw_duration)) %>%
  mutate(effort_compiled = avg_tracing_score_scaled + mean_intensity_scaled + draw_duration_scaled)
```

```{r}
full_model_with_effort_combined <- glmer(cbind(num_correct, num_incorrect) ~ condition*scale(age_numeric)*site +
                      scale(avg_tracing_score) +
                      scale(effort_compiled) + 
                      (condition | unique_ids) +
                      (condition*scale(age_numeric) | category),
      data = d, family='binomial', control=glmerControl(optCtrl=list(maxfun=20000),optimizer=c("bobyqa")))


full_model_with_effort_combined_out = summary(full_model_with_effort_combined)

```

```{r}
# correct_by_effort_and_site <- glmer(cbind(num_correct, num_incorrect) ~ scale(draw_duration)*site + age_numeric + condition +
#                       (1 | unique_ids) +
#                       (scale(age_numeric) | category),
#       data = d,family='binomial', control=glmerControl(optCtrl=list(maxfun=20000),optimizer=c("bobyqa")))
# 
# correct_by_effort_and_site_out = summary(correct_by_effort_and_site)
```


```{r}
drawing_duration_by_site <- lmer(scale(draw_duration) ~ site*condition + 
                      (1 | unique_ids) +
                      (scale(age_numeric) | category),
      data = d)

drawing_duration_out = summary(drawing_duration_by_site)

papaja::apa_table(drawing_duration_out$coefficients, caption='"Model coefficients from a linear mixed mode predicting the how long children spent on each drawing as a function of site (Beijing vs. San Jose) and drawing condition.')
```
```{r}
num_strokes_by_site <- lmer(scale(num_strokes) ~ site*condition  + 
                      (1 | unique_ids) +
                      (scale(age_numeric) | category),
      data = d)

num_strokes_by_site_out = summary(num_strokes_by_site)
papaja::apa_table(num_strokes_by_site_out$coefficients, caption='"Model coefficients from a linear mixed mode predicting the number of strokes children made on each drawing as a function of site (Beijing vs. San Jose) and drawing condition.')
```

```{r}
intensity_by_site <- lmer(scale(mean_intensity) ~ site*condition  + 
                      (1 | unique_ids) +
                      (scale(age_numeric) | category),
      data = d)

intensity_by_site_out = summary(intensity_by_site)
papaja::apa_table(intensity_by_site_out$coefficients, caption='"Model coefficients from a linear mixed mode predicting the amount of "ink" (i.e., average drawing intensity) children used on each drawing as a function of site (Beijing vs. San Jose) and drawing condition.')
```

```{r}
effort_by_site <- lmer(scale(effort_compiled) ~ site*age_numeric  + 
                      (1 | unique_ids) +
                      (scale(age_numeric) | category),
      data = d)

effort_out = summary(effort_by_site)
papaja::apa_table(effort_out$coefficients, caption='"Model coefficients from a linear mixed mode predicting the amount of overall effort children used on each drawing (sum of z-scored effort covariates -- ink, strokes, and time) as a function of site (Beijing vs. San Jose) and drawing condition.')
```


```{r}
indiv_by_acc <- d %>%
  group_by(unique_ids, avg_tracing_score_scaled) %>%
  summarize(prop_correct = mean(prop_correct)) 

cor.test(indiv_by_acc$avg_tracing_score_scaled, indiv_by_acc$prop_correct)
```


```{r}
covariates <- d %>% select(num_strokes, draw_duration, mean_intensity, avg_tracing_score)

covariates_cor_table = round(cor(covariates),3)
rownames(covariates_cor_table)<-c('Number of strokes','Drawing duration (s)','Mean intensity','Average tracing score')
colnames(covariates_cor_table)<-c('Number of strokes','Drawing duration (s)','Mean intensity','Average tracing score')

papaja::apa_table(covariates_cor_table, caption="Pearsons corellation coefficients for all effort covariates (estimed for each drawing) and tracing scores (estimated by each participant).")
...
```{r}
tracing_score_stats = summary((lm(data = tracing_scores_indiv, avg_tracing_score ~ scale(age_numeric)*site)))


papaja::apa_table(tracing_score_stats$coefficients, caption='"Model coefficients from a  linear regression predicting the average tracing score of each child as a function of their age and the site at which they participated.')
...
ggplot(tracing_scores, aes(x=age_numeric,y=mean, col=site)) +
  ...
  geom_jitter(data = tracing_scores_indiv, aes(x=age_numeric, y=avg_tracing_score), width=.2, height=.01, alpha=.5, size=.5) +
  theme_few(base_size = base_size_chosen) +
  labs(x='Age', y='Tracing score') +
  theme(legend.position = "none", aspect.ratio=1.3) +
  geom_smooth(span = 20,alpha=smooth_alpha, aes(group =site), size=.5)  +
  scale_x_continuous(breaks=c(4,5,6,7,8,9)) +
  facet_grid(~site) +
  ylim(-.1,4.1) +
...
<!-- # Appendix Table B1: drawing duration -->
<!-- # Appendix Table B2: intensity -->
<!-- # Appendix Table B3: strokes -->
<!-- # Appendix Table B4: compiled effort -->
