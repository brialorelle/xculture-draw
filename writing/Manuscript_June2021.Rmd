...
In follow-up analyses, we investigated potential sources of variation that might explain these site-related differences. 
We initially hypothesized that any differences in drawing recognizability between sites could be mediated by the amount of effort children expended while drawing or their tracing abilities. 
...
By contrast, children's tracing abilities did not vary across sites: Figure \ref{fig:tracing} shows the average tracing score at each age at each site. While children's tracing abilities increased steadily with age, as in prior work [@long2021parallel], children from both sites produced equally good tracings.

We then included children's average tracing scores and effort covariates measured for each drawing (average intensity, number of strokes used, and time spent drawing) as fixed effects into a second generalized linear mixed-effects model (see Statistical Models). If effort and/or tracing ability is sufficient to account for the differences between sites, we reasoned that we should no longer observe a main effect of geographical site on drawing recognizability in this expanded statistical model. Contra this hypothesis, we still observed a significant effect of geographical site (see Table 2), even having accounted for the effect of individual differences in tracing ability. We also found that the amount of time a child spent drawing negatively predicted recognizability -- indicating that spending more time on a drawing did not necessarily result in a more recognizable drawing. Together, these results suggest that these site differences in the ability to produce recognizable drawings are not explained by differences in effort or tracing ability. 

**JEFAN QUERY: I noticed that while draw duration was a negative predictor of recognizability, number of strokes was a positive predictor. We comment on the former, but not the latter. Could it be potentially confusing to a reader if we do not mention the latter?**
...
p1=ggplot(draw_duration, aes(age_numeric,mean, color=condition)) +
  ...
  scale_color_viridis(name="", discrete=TRUE, option='D', begin=.2, end=.8) +
  theme(legend.position = "bottom") + 
  ylim(0,30) +
  facet_grid(~site)
...
ggplot(tracing_scores, aes(x=age_numeric,y=mean, col=site)) +
  ...
  geom_jitter(data = tracing_scores_indiv, aes(x=age_numeric, y=avg_tracing_score), width=.2, height=.01, alpha=.5) +
  theme_few(base_size = base_size_chosen) +
...
We found steady improvement from 4-9 years of age in both groups of children, replicating prior work conducted in a field setting [@long2021parallel].
...
On the other hand, we discovered that drawing recognizability differed between sites, with children in the Beijing group having produced more recognizable drawings than those in the San Jose group. 
...
Instead, we found that children across the two sites were indistinguishable with respect to tracing ability on average, although individual tracing scores were strong predictors of the recognizablity of their own drawings. 
...
However, we also found that these effort measures could not account for differences in drawing recognizability between groups. 
In exploratory analyses, we also discovered that the groups differed with respect to which visual concepts were easier for children to convey in their drawings, perhaps reflecting different patterns of exposure to these concepts. 
**JEFAN QUERY: The previous six sentences were edited for clarity but are redundant with similar sentences in the results section. Perhaps it would be more effective to consolidate them (in Results?) and summarize them more concisely here?**
While such group differences are intriguing, they are consistent with a number of other potential explanations.
Further systematic investigation within and between geographical, socioeconomic, and cultural contexts will be crucial for producing more precise characterization of developmental variability in drawing behavior, itself critical to strongly evaluate causal theories of such variation [@amir2020cross, @carstensen2021investigating, @winner1989can, @kuwabara2016cultural].
We have made our drawing dataset publicly available to contribute to this more cumulative effort and to reduce barriers to investigation of other aspects of these drawings other than their recognizability, such as which visual features children prioritized in their drawings, the order in which they drew them, as well as variation in visual style [@gernhardt2015cultural, @senzaki2014holistic]. 

Another natural question for future work concerns the relationship between the kind of experience children have with these visual concepts and the way they produce drawings of them. 
For example, if some children are exposed to a wide variety of illustrations of some concepts but not others (e.g., in books or other media), they may also be able to produce more recognizable drawings of those concepts. 
Conversely, if some children spend more time drawing certain visual concepts but not others (e.g., in school or at home), they may also be better at recognizing graphical representations of them in other contexts, and perhaps explicitly identifying their diagnostic features. 
Prior work has found that older children not only produce more recognizable drawings of object categories, but also tend to be better at recognizing other children's drawings [@long2021parallel], providing evidence that visual production and recognition abilities are related throughout childhood at the group level.
However, further investigation will be necessary for understanding what kinds of experience are responsible for concurrent developmental changes in these two behaviors at the individual level.

**JEFAN QUERY: We currently do not have a paragraph that expands on the null effect of cue type. Though perhaps we could, for "balance?"**

In sum, our findings advance our understanding of the relative impact of age, task settings, and geographical context on the ability of children to communicate their knowledge about familiar visual concepts by drawing. 
More broadly, we believe our approach to concurrently quantifying multiple sources of variation in the context of rich, naturalistic behaviors will lead to more robust and unified theories of cognitive development. 

**JEFAN QUERY: Is there any way to hijack this APA template to force figures/tables to appear in the text close to where they are mentioned, rather than all the way at the end?**
