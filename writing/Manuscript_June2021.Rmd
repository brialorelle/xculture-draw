...
```{r}
site_difference <- lmer(prop_correct ~ scale(age_numeric)^2*site +
                      (1 | unique_ids) +
                      (1 | category),
      data = d)
...
While a long literature has sought to examine variation in cognitive abilities across cultural contexts, we note that this is there is substantial variation within different geographical locations and countries that often goes unmeasured.  Drawings from children within the same country -- or even area code --  may differ in the very same ways children's drawings differ in this dataset. For example,  children across could reasonably differ in the amount of practice they have drawing common object categories. If so, then children who spend more time drawing may simply have more practice communicating about the diagnostic features of different object categories. In future work, surveys that assesses the amount of time that children at these two sites -- as well as many others -- spend drawing outside of these experimental contexts could address this hypothesis.  
